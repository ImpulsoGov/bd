...
SELECT v3.estabelecimento_cnes,
   ...
   FROM ( 
   SELECT row_number() OVER (PARTITION BY v2.gestante_nome, v2.gestante_data_de_nascimento,v2.gestante_dum_primeiro_atendimento ORDER BY v2.atendimento_data DESC) AS r,
            v2.atendimento_data,
			...
			count(v2.atendimento_data) filter (where v2.dt_registro_parto_mais_recente is not null) OVER (PARTITION BY v2.gestante_nome, v2.gestante_data_de_nascimento,v2.gestante_dum_primeiro_atendimento) > 0 as possui_registro_parto,
			count(v2.atendimento_data) filter (where v2.dt_registro_aborto_mais_recente is not null) OVER (PARTITION BY v2.gestante_nome, v2.gestante_data_de_nascimento,v2.gestante_dum_primeiro_atendimento) > 0 as possui_registro_aborto,
           ...
           SELECT v1.atendimento_data,
                    ...
                    /* Retorna DUM do primeiro atendimento da gestação */
                    case 
                    	when v1.atendimento_data >= v1.dt_registro_parto or v1.atendimento_data >= (v1.gestante_dpp + '14 days'::interval) or v1.atendimento_data >= v1.dt_registro_aborto
                    	then (array_agg(v1.gestante_dum) FILTER (WHERE v1.gestante_dum <> '3000-12-31'::date and v1.atendimento_data >= v1.dt_registro_parto) OVER (PARTITION BY v1.gestante_nome, v1.gestante_data_de_nascimento ORDER BY v1.atendimento_data))[1] 
                    ...
                    end gestante_dum_primeiro_atendimento,
                    /* Retorna data do primeiro atendimento de cada gestação */
                    case 
                    	when v1.atendimento_data >= v1.dt_registro_parto or v1.atendimento_data >= (v1.gestante_dpp + '14 days'::interval) or v1.atendimento_data >= v1.dt_registro_aborto
                    	then (array_agg(v1.atendimento_data) FILTER (WHERE v1.gestante_dum <> '3000-12-31'::date and v1.atendimento_data >= v1.dt_registro_parto) OVER (PARTITION BY v1.gestante_nome, v1.gestante_data_de_nascimento ORDER BY v1.atendimento_data))[1] 
                    	else (array_agg(v1.atendimento_data) FILTER (WHERE v1.gestante_dum <> '3000-12-31'::date) OVER (PARTITION BY v1.gestante_nome, v1.gestante_data_de_nascimento ORDER BY v1.atendimento_data))[1] 
                    end atendimento_primeiro_data,
                    /* Retorna data de parto na gestação se houver registro */
                    case 
                    	when v1.gestante_dum = '3000-12-31'::date and v1.atendimento_data <= v1.dt_registro_parto
                    	then (array_agg(v1.dt_registro_parto) FILTER (WHERE v1.gestante_dum <> '3000-12-31'::date and v1.atendimento_data <= v1.dt_registro_parto) OVER (PARTITION BY v1.gestante_nome, v1.gestante_data_de_nascimento ORDER BY v1.atendimento_data))[1] 
                    	when v1.dt_registro_parto <= v1.gestante_dum or (v1.gestante_dum = '3000-12-31'::date and v1.atendimento_data >= v1.dt_registro_parto)
                    	then null
                    ...
                    end dt_registro_parto_mais_recente,
                    /* Retorna data de aborto na gestação se houver registro */
                    case 
                    	when v1.gestante_dum = '3000-12-31'::date
                    	then (array_agg(v1.dt_registro_aborto) FILTER (WHERE v1.gestante_dum <> '3000-12-31'::date and v1.atendimento_data <= v1.dt_registro_aborto) OVER (PARTITION BY v1.gestante_nome, v1.gestante_data_de_nascimento ORDER BY v1.atendimento_data))[1] 
                    	when v1.dt_registro_aborto <= v1.gestante_dum
                    	then null
                    ...
                    end dt_registro_aborto_mais_recente
                   FROM ( 
                   SELECT DISTINCT tfai.co_seq_fat_atd_ind,
                            ...
                            NULLIF(concat(tfcd.no_logradouro, ', ', tfcd.nu_num_logradouro), ', '::text) AS gestante_endereco,
                            	/* Retorna a DUM informada na consulta */
                                CASE
                            ...
                            tfai.nu_idade_gestacional_semanas AS gestante_idade_gestacional_atendimento,
                                ( SELECT max(tdtempoparto.dt_registro)
				                  ...
				                  WHERE (tfaiparto.co_fat_cidadao_pec IN ( SELECT tfcparto.co_seq_fat_cidadao_pec
				                          ...
				                          ) AS dt_registro_parto,
				                ( SELECT max(tdtempoaborto.dt_registro)
                  ...
                  WHERE (tfaiaborto.co_fat_cidadao_pec IN ( SELECT tfparto.co_seq_fat_cidadao_pec
                          ...
                          WHERE tfparto.no_cidadao::text = tfcp.no_cidadao::text AND tfparto.co_dim_tempo_nascimento::text = tfcp.co_dim_tempo_nascimento::text)) AND ((tdciapaborto.nu_ciap::text = ANY (ARRAY['W82'::text, 'W83'::text])) OR (tdcidaborto.nu_cid::text = ANY (ARRAY['O02'::text, 'O03'::text, 'O05'::text, 'O06'::text, 'O04'::text, 'Z303'::text])))
                          ) AS dt_registro_aborto
  ...
  WHERE tfcatendimento.no_cidadao::text = tfcp.no_cidadao::text AND tfcatendimento.co_dim_tempo_nascimento = tfcp.co_dim_tempo_nascimento))
                          ...
                          and tdt.dt_registro >= ((( SELECT
							CASE
							 WHEN date_part('month'::text, '2022-08-31'::date) >= 1::double precision AND date_part('month'::text, '2022-08-31'::date) <= 4::double precision THEN concat(date_part('year'::text, ('2022-08-31'::date - '365 days'::interval)::date), '-09-01')
							 WHEN date_part('month'::text, '2022-08-31'::date) >= 5::double precision AND date_part('month'::text, '2022-08-31'::date) <= 8::double precision THEN concat(date_part('year'::text, '2022-08-31'::date), '-01-01')
							 WHEN date_part('month'::text, '2022-08-31'::date) >= 9::double precision AND date_part('month'::text, '2022-08-31'::date) <= 12::double precision THEN concat(date_part('year'::text, '2022-08-31'::date), '-05-01')
							 ELSE NULL::text
							END::date AS date)) - '294 days'::interval) ) v1) v2
							) v3
						WHERE v3.r = 1
