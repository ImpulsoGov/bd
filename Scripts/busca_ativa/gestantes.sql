-- lista_nominal_gestantesvs02 source

CREATE MATERIALIZED VIEW lista_nominal_gestantes
TABLESPACE pg_default
AS SELECT v3.estabelecimento_cnes,
   ...
   FROM ( SELECT
           ...
           FROM ( SELECT v1.atendimento_data,
                    ...
                    (array_agg(v1.gestante_dpp) FILTER (WHERE v1.gestante_dpp IS NOT NULL) OVER (PARTITION BY v1.gestante_nome, v1.gestante_data_de_nascimento, v1.gestante_dum ORDER BY v1.atendimento_data))[1] AS gestante_primeira_dpp,
                        CASE
                            WHEN v1.atendimento_data >= ANY (ARRAY[v1.dt_registro_parto::timestamp without time zone, v1.gestante_dpp + '14 days'::interval, v1.dt_registro_aborto::timestamp without time zone]) THEN (array_agg(v1.gestante_dum) FILTER (WHERE v1.gestante_dum <> '3000-12-31'::date AND (v1.atendimento_data >= ANY (ARRAY[v1.dt_registro_parto::timestamp without time zone, v1.gestante_dpp + '14 days'::interval, v1.dt_registro_aborto::timestamp without time zone]))) OVER (PARTITION BY v1.gestante_nome, v1.gestante_data_de_nascimento ORDER BY v1.atendimento_data))[1]
                     ...
                     * não haja retorna a primeira data atendimento identificada */
                        CASE
                            WHEN v1.atendimento_data >= ANY (ARRAY[v1.dt_registro_parto::timestamp without time zone, v1.gestante_dpp + '14 days'::interval, v1.dt_registro_aborto::timestamp without time zone]) THEN (array_agg(v1.atendimento_data) FILTER (WHERE v1.gestante_dum <> '3000-12-31'::date AND (v1.atendimento_data >= ANY (ARRAY[v1.dt_registro_parto::timestamp without time zone, v1.gestante_dpp + '14 days'::interval, v1.dt_registro_aborto::timestamp without time zone]))) OVER (PARTITION BY v1.gestante_nome, v1.gestante_data_de_nascimento ORDER BY v1.atendimento_data))[1]
...
END::date - '294 days'::interval))
                          ORDER BY tdt.dt_registro) v1) v2) v3
  WHERE v3.r = 1
WITH DATA;