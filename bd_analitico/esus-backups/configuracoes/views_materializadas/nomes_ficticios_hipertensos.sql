...
CREATE MATERIALIZED VIEW configuracoes.nomes_ficticios_hipertensos
...
AS WITH geracao_nomes AS (
         WITH base AS (
                 SELECT row_number() OVER (PARTITION BY 0::integer) AS sequencia,
                   ...
                   FROM generate_series(1, 20000) s(a)
                     CROSS JOIN ( SELECT ( SELECT array_agg(DISTINCT "substring"(tb_1.cidadao_nome::text, '^[^ ]+'::text)) AS nome
                                   FROM dados_nominais_ba_jacobina.lista_nominal_hipertensos tb_1) AS primeiro_nome,
