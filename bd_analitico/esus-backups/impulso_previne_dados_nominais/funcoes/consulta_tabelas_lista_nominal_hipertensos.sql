CREATE OR REPLACE FUNCTION impulso_previne_dados_nominais.consulta_tabelas_lista_nominal_hipertensos()
 RETURNS TABLE(municipio_id_sus character varying, quadrimestre_atual text, realizou_afericao_ultimos_6_meses boolean, dt_afericao_pressao_mais_recente character varying, realizou_consulta_ultimos_6_meses boolean, dt_consulta_mais_recente date, co_seq_fat_cidadao_pec text, cidadao_cpf character varying, cidadao_cns character varying, cidadao_nome character varying, cidadao_nome_social character varying, cidadao_sexo text, dt_nascimento date, estabelecimento_cnes_atendimento text, estabelecimento_cnes_cadastro text, estabelecimento_nome_atendimento text, estabelecimento_nome_cadastro text, equipe_ine_atendimento text, equipe_ine_cadastro text, equipe_ine_procedimento text, equipe_nome_atendimento text, equipe_nome_cadastro text, equipe_nome_procedimento text, acs_nome_cadastro text, acs_nome_visita text, profissional_nome_atendimento text, profissional_nome_procedimento text, possui_hipertensao_autorreferida boolean, possui_hipertensao_diagnosticada boolean, data_ultimo_cadastro date, dt_ultima_consulta date, se_faleceu integer, se_mudou integer, criacao_data timestamp with time zone, atualizacao_data timestamp with time zone)
 LANGUAGE plpgsql
AS $function$
DECLARE
    r record;
begin
	...
	LOOP
		...
		municipio_id_sus, 
		quadrimestre_atual, 
		realizou_afericao_ultimos_6_meses, 
		...
		possui_hipertensao_autorreferida,
