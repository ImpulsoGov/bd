         ...
         WITH base AS (
                 SELECT DISTINCT b_1.municipio_id_sus,
                    ...
                    b_1.profissional_nome_atendimento,
                        CASE
                            WHEN COALESCE(b_1.data_dum, '3000-12-31'::date) <> '3000-12-31'::date THEN b_1.data_dum
                        ...
                        CASE
                            WHEN COALESCE(b_1.data_dum, '3000-12-31'::date) <> '3000-12-31'::date THEN (b_1.data_dum + '294 days'::interval)::date
                        ...
                        CASE
                            WHEN COALESCE(b_1.data_dum, '3000-12-31'::date) <> '3000-12-31'::date THEN (CURRENT_DATE - b_1.data_dum) / 7
         ...
         SELECT vd.municipio_id_sus,
            ...
            LEAST(ra.menor_data_registro_aborto::timestamp without time zone, vd.primeira_data_dpp)::date AS data_fim_primeira_gestacao,
                ...
                CASE
                    ...
                    WHEN rp.diff_dias_primeio_ultimo_parto = 0 THEN 'apenas_um_parto'::text
                ...
                CASE
                    ...
                    WHEN ra.diff_dias_primeio_ultimo_aborto = 0 THEN 'apenas_um_aborto'::text
                    ELSE 'possibilidade_apenas_um_aborto_ou_erro_registro'::text
                END AS tipo_registro_aborto
           FROM validacao_dum vd
             LEFT JOIN validacao_registros_parto rp ON rp.chave_gestante::text = vd.chave_gestante::text AND rp.municipio_id_sus::text = vd.municipio_id_sus::text
             LEFT JOIN validacao_registros_aborto ra ON ra.chave_gestante::text = vd.chave_gestante::text AND ra.municipio_id_sus::text = vd.municipio_id_sus::text
