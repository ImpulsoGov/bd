...
AS WITH dados_transmissoes_recentes AS (
         ...
         SELECT tb1.municipio_id_sus,
            ...
            -- data da consulta mais recente de diabetes
            tb1.dt_consulta_mais_recente,
                ...
                CASE
                    WHEN tb1.realizou_afericao_ultimos_6_meses AND tb1.realizou_consulta_ultimos_6_meses THEN 'Consulta e aferição de PA em dia'::text
                    WHEN tb1.realizou_afericao_ultimos_6_meses IS FALSE AND tb1.realizou_consulta_ultimos_6_meses IS FALSE THEN 'Consulta e aferição de PA a fazer'::text
                    WHEN tb1.realizou_afericao_ultimos_6_meses AND tb1.realizou_consulta_ultimos_6_meses IS FALSE THEN 'Apenas consulta a fazer'::text
                    WHEN tb1.realizou_afericao_ultimos_6_meses IS FALSE AND tb1.realizou_consulta_ultimos_6_meses THEN 'Apenas aferição de PA a fazer'::text
                    ELSE NULL::text
 ...
 SELECT md5(((((COALESCE(aux.municipio_id_sus, ''::text) || COALESCE(aux.cidadao_cpf, ''::text)) || COALESCE(aux.cidadao_nome, ''::text)) || COALESCE(aux.cidadao_cpf_dt_nascimento, ''::text)) || COALESCE(aux.cidadao_cns, ''::text)) || COALESCE(aux.dt_ultima_consulta::text, ''::text)) AS id,
    aux.municipio_id_sus,
    aux.municipio_uf,
    aux.quadrimestre_atual,
    aux.realizou_afericao_ultimos_6_meses,
    aux.dt_afericao_pressao_mais_recente,
    aux.realizou_consulta_ultimos_6_meses,
    aux.dt_consulta_mais_recente,
    aux.prazo_proxima_afericao_pa,
    aux.prazo_proxima_consulta,
    aux.consulta_e_afericao_em_dia,
    aux.status_em_dia,
    aux.status_usuario,
    aux.identificacao_condicao_hipertensao,
    aux.cidadao_cpf,
    aux.cidadao_cpf_dt_nascimento,
    aux.cidadao_cns,
    aux.cidadao_nome,
    aux.cidadao_nome_social,
    aux.cidadao_sexo,
    aux.dt_nascimento,
    aux.cidadao_idade,
    aux.cidadao_faixa_etaria,
    aux.estabelecimento_cnes_atendimento,
    aux.estabelecimento_cnes,
    aux.estabelecimento_nome_atendimento,
    aux.estabelecimento_nome,
    aux.equipe_ine_atendimento,
    aux.equipe_ine_cadastro,
    aux.equipe_nome_atendimento,
    aux.equipe_nome_cadastro,
    aux.acs_nome_cadastro,
    aux.acs_nome_visita,
    aux.possui_hipertensao_autorreferida,
    aux.possui_hipertensao_diagnosticada,
    aux.apenas_autorreferida,
    aux.diagnostico_clinico,
    aux.data_ultimo_cadastro,
    aux.dt_ultima_consulta,
    aux.se_faleceu,
    aux.se_mudou,
    aux.criacao_data,
    aux.atualizacao_data,
    aux.dt_registro_producao_mais_recente
   FROM aux
    WITH DATA;