-- impulso_previne.area_logada_perc_usuarios_ativos_semanal source

CREATE MATERIALIZED VIEW impulso_previne.area_logada_perc_usuarios_ativos_semanal
...
AS WITH area_logada_eventos_atividade_retencao_usuarios AS (
         ...
         SELECT 'semana'::text AS periodo_referencia,
            COALESCE(ear.semana_ativo, ear.semana_primeiro_acesso) AS data_periodo,
            count(DISTINCT
                CASE
          ...
          GROUP BY 'semana'::text, (COALESCE(ear.semana_ativo, ear.semana_primeiro_acesso))
        )
...
-- View indexes:
CREATE INDEX area_logada_perc_usuarios_ativos_semanal_data_periodo_idx ON impulso_previne.area_logada_perc_usuarios_ativos_semanal USING btree (data_periodo);