-- impulso_previne.area_logada_usuarios_ativos_bisemanal_cohort source

CREATE MATERIALIZED VIEW impulso_previne.area_logada_usuarios_ativos_bisemanal_cohort
...
AS WITH area_logada_eventos_atividade_retencao_usuarios AS (
         ...
         SELECT 'bi-semanal'::text AS periodo_referencia,
            COALESCE(ear.bisemanal_ativo, ear.bisemanal_primeiro_acesso) AS data_periodo,
                CASE
                    WHEN ear.mes_primeiro_acesso = ANY (ARRAY['2022-11-01'::date, '2022-12-01'::date]) THEN 'Cohort 2022'::text
                    WHEN ear.mes_primeiro_acesso = ANY (ARRAY['2023-01-01'::date, '2023-02-01'::date, '2023-03-01'::date]) THEN 'Cohort Jan-Mar/2023'::text
                    WHEN ear.mes_primeiro_acesso = ANY (ARRAY['2023-04-01'::date, '2023-05-01'::date, '2023-06-01'::date]) THEN 'Cohort Abr-Jun/2023'::text
                    WHEN ear.mes_primeiro_acesso = ANY (ARRAY['2023-07-01'::date, '2023-08-01'::date, '2023-09-01'::date]) THEN 'Cohort Jul-Set/2023'::text
                    ELSE NULL::text
                END AS cohort_primeiro_acesso,
            count(DISTINCT
                CASE
           ...
           FROM area_logada_eventos_atividade_retencao_usuarios ear
          GROUP BY 'bi-semanal'::text, (COALESCE(ear.bisemanal_ativo, ear.bisemanal_primeiro_acesso)), (
                CASE
                    WHEN ear.mes_primeiro_acesso = ANY (ARRAY['2022-11-01'::date, '2022-12-01'::date]) THEN 'Cohort 2022'::text
                    WHEN ear.mes_primeiro_acesso = ANY (ARRAY['2023-01-01'::date, '2023-02-01'::date, '2023-03-01'::date]) THEN 'Cohort Jan-Mar/2023'::text
                    WHEN ear.mes_primeiro_acesso = ANY (ARRAY['2023-04-01'::date, '2023-05-01'::date, '2023-06-01'::date]) THEN 'Cohort Abr-Jun/2023'::text
         ...
         SELECT ba.periodo_referencia,
            ba.data_periodo,
            ba.cohort_primeiro_acesso,
            ba.novos_usuarios,
            ba.usuarios_ativos,
            sum(ba.novos_usuarios) OVER (PARTITION BY ba.cohort_primeiro_acesso ORDER BY ba.data_periodo ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS total_usuarios_acc,
            ba.usuarios_ativos::numeric * 1.00 / sum(ba.novos_usuarios) OVER (PARTITION BY ba.cohort_primeiro_acesso ORDER BY ba.data_periodo ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS perc_usuarios_ativos,
            ba.novos_usuarios_coordenacao,
            ba.usuarios_ativos_coordenacao,
            sum(ba.novos_usuarios_coordenacao) OVER (PARTITION BY ba.cohort_primeiro_acesso ORDER BY ba.data_periodo ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS total_usuarios_coordenacao_acc,
            ba.usuarios_ativos_coordenacao::numeric * 1.00 / sum(ba.novos_usuarios_coordenacao) OVER (PARTITION BY ba.cohort_primeiro_acesso ORDER BY ba.data_periodo ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS perc_usuarios_ativos_coordenacao,
            first_value(ba.data_periodo) OVER (PARTITION BY ba.cohort_primeiro_acesso ORDER BY ba.usuarios_ativos_coordenacao DESC, ba.data_periodo DESC) AS quizena_max_usuarios_ativos_coordenacao,
            first_value(ba.usuarios_ativos_coordenacao) OVER (PARTITION BY ba.cohort_primeiro_acesso ORDER BY ba.data_periodo DESC) AS ultima_quizena_usuarios_ativos_coordenacao,
            first_value(ba.data_periodo) OVER (PARTITION BY ba.cohort_primeiro_acesso ORDER BY ba.data_periodo DESC) AS ultima_quizena,
            CURRENT_DATE AS criacao_data
           FROM base_atividade ba
        )
 SELECT a.periodo_referencia,
    a.data_periodo,
    a.cohort_primeiro_acesso,
    a.novos_usuarios,
    a.usuarios_ativos,
    ...
    a.ultima_quizena,
    lag(a.data_periodo) OVER (PARTITION BY a.cohort_primeiro_acesso ORDER BY a.data_periodo) AS quinzena_lw,
    a.criacao_data,
    lag(a.usuarios_ativos_coordenacao) OVER (PARTITION BY a.cohort_primeiro_acesso ORDER BY a.data_periodo) AS usuarios_ativos_coordenacao_lw,
    lag(a.perc_usuarios_ativos_coordenacao) OVER (PARTITION BY a.cohort_primeiro_acesso ORDER BY a.data_periodo) AS perc_usuarios_ativos_coordenacao_lw,
    (a.usuarios_ativos_coordenacao - lag(a.usuarios_ativos_coordenacao) OVER (PARTITION BY a.cohort_primeiro_acesso ORDER BY a.data_periodo))::numeric * 1.00 / lag(a.usuarios_ativos_coordenacao) OVER (PARTITION BY a.cohort_primeiro_acesso ORDER BY a.data_periodo)::numeric AS var_ultima_quizena_usuarios_ativos_coordenacao,
    max(a.usuarios_ativos_coordenacao) OVER (PARTITION BY a.cohort_primeiro_acesso) AS max_usuarios_ativos_coordenacao,
    max(
        CASE
            ...
            ELSE NULL::bigint
        END) OVER (PARTITION BY a.cohort_primeiro_acesso) AS max_usuarios_ativos_coordenacao_ultimos3meses,
    (a.usuarios_ativos_coordenacao - max(
        CASE
            WHEN a.data_periodo >= (date_trunc('month'::text, CURRENT_DATE::timestamp with time zone)::date - '3 mons'::interval) THEN a.usuarios_ativos_coordenacao
            ELSE NULL::bigint
        END) OVER (PARTITION BY a.cohort_primeiro_acesso))::numeric * 1.00 / max(
        CASE
            WHEN a.data_periodo >= (date_trunc('month'::text, CURRENT_DATE::timestamp with time zone)::date - '3 mons'::interval) THEN a.usuarios_ativos_coordenacao
            ELSE NULL::bigint
        END) OVER (PARTITION BY a.cohort_primeiro_acesso)::numeric AS var_max_usuarios_ativos_coordenacao,
    max(a.perc_usuarios_ativos_coordenacao) OVER (PARTITION BY a.cohort_primeiro_acesso) AS max_perc_usuarios_ativos_coordenacao,
    max(
        CASE
            ...
            ELSE NULL::numeric
        END) OVER (PARTITION BY a.cohort_primeiro_acesso) AS max_perc_usuarios_ativos_coordenacao_ultimos3meses,
        CASE
            WHEN a.data_periodo = a.ultima_quizena THEN true
            ELSE false
...
-- View indexes:
CREATE INDEX area_logada_usuarios_ativos_bisemanal_cohort_periodo_referencia ON impulso_previne.area_logada_usuarios_ativos_bisemanal_cohort USING btree (periodo_referencia, data_periodo, cohort_primeiro_acesso);