-- impulso_previne.area_logada_usuarios_ativos_bisemanal_geral source

CREATE MATERIALIZED VIEW impulso_previne.area_logada_usuarios_ativos_bisemanal_geral
...
AS WITH area_logada_eventos_atividade_retencao_usuarios AS (
         ...
         SELECT 'bi-semanal'::text AS periodo_referencia,
          ...
          WHERE ear.municipio::text <> ALL (ARRAY['Pirenópolis - GO'::character varying::text, 'Carolina - MA'::character varying::text, 'José Gonçalves de Minas - MG'::character varying::text, 'Amparo - SP'::character varying::text])
          GROUP BY (ear.bisemanal_ativo::text), (COALESCE(ear.bisemanal_ativo, ear.bisemanal_primeiro_acesso))
        ), aux1 AS (
         SELECT ba.periodo_referencia,
            ba.data_periodo,
            ba.novos_usuarios,
            ba.usuarios_ativos,
            sum(ba.novos_usuarios) OVER (ORDER BY ba.data_periodo ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS total_usuarios_acc,
            ba.usuarios_ativos::numeric * 1.00 / sum(ba.novos_usuarios) OVER (ORDER BY ba.data_periodo ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS perc_usuarios_ativos,
            ba.novos_usuarios_coordenacao,
            ba.usuarios_ativos_coordenacao,
            sum(ba.novos_usuarios_coordenacao) OVER (ORDER BY ba.data_periodo ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS total_usuarios_coordenacao_acc,
            ba.usuarios_ativos_coordenacao::numeric * 1.00 / sum(ba.novos_usuarios_coordenacao) OVER (ORDER BY ba.data_periodo ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS perc_usuarios_ativos_coordenacao,
            first_value(ba.data_periodo) OVER (ORDER BY ba.usuarios_ativos_coordenacao DESC, ba.data_periodo DESC) AS quizena_max_usuarios_ativos_coordenacao,
            first_value(ba.usuarios_ativos_coordenacao) OVER (ORDER BY ba.data_periodo DESC) AS ultima_quizena_usuarios_ativos_coordenacao,
            first_value(ba.data_periodo) OVER (ORDER BY ba.data_periodo DESC) AS ultima_quizena,
            CURRENT_DATE AS criacao_data
 ...
 SELECT a.periodo_referencia,
    ...
    a.ultima_quizena,
    lag(a.data_periodo) OVER (ORDER BY a.data_periodo) AS quinzena_lw,
    a.criacao_data,
    lag(a.usuarios_ativos_coordenacao) OVER (ORDER BY a.data_periodo) AS usuarios_ativos_coordenacao_lw,
    lag(a.perc_usuarios_ativos_coordenacao) OVER (ORDER BY a.data_periodo) AS perc_usuarios_ativos_coordenacao_lw,
    (a.usuarios_ativos_coordenacao - lag(a.usuarios_ativos_coordenacao) OVER (ORDER BY a.data_periodo))::numeric * 1.00 / lag(a.usuarios_ativos_coordenacao) OVER (ORDER BY a.data_periodo)::numeric AS var_ultima_quizena_usuarios_ativos_coordenacao,
    max(a.usuarios_ativos_coordenacao) OVER () AS max_usuarios_ativos_coordenacao,
    max(
        CASE
            ...
            ELSE NULL::bigint
        END) OVER () AS max_usuarios_ativos_coordenacao_ultimos3meses,
    (a.usuarios_ativos_coordenacao - max(
        CASE
            WHEN a.data_periodo >= (date_trunc('month'::text, CURRENT_DATE::timestamp with time zone)::date - '3 mons'::interval) THEN a.usuarios_ativos_coordenacao
            ELSE NULL::bigint
        END) OVER ())::numeric * 1.00 / max(
        CASE
            WHEN a.data_periodo >= (date_trunc('month'::text, CURRENT_DATE::timestamp with time zone)::date - '3 mons'::interval) THEN a.usuarios_ativos_coordenacao
            ELSE NULL::bigint
        END) OVER ()::numeric AS var_max_usuarios_ativos_coordenacao,
    max(a.perc_usuarios_ativos_coordenacao) OVER () AS max_perc_usuarios_ativos_coordenacao,
    max(
        CASE
            ...
            ELSE NULL::numeric
        END) OVER () AS max_perc_usuarios_ativos_coordenacao_ultimos3meses,
        CASE
...
-- View indexes:
CREATE INDEX area_logada_usuarios_ativos_bisemanal_geral_periodo_referencia_ ON impulso_previne.area_logada_usuarios_ativos_bisemanal_geral USING btree (periodo_referencia, data_periodo);