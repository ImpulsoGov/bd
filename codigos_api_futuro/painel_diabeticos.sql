...
CREATE MATERIALIZED VIEW impulso_previne_dados_nominais.painel_diabeticos
...
AS SELECT db.municipio_id_sus,
    ...
    count(DISTINCT db.cidadao_nome || db.dt_nascimento) AS total_usuarios_com_diabetes,
    count(DISTINCT
        CASE
            WHEN db.status_em_dia = 'Em dia'::text THEN db.cidadao_nome || db.dt_nascimento
            ELSE NULL::text
        END) AS total_com_consulta_solicitacao_hbg_em_dia,
    count(DISTINCT
        CASE
   ...
   FROM impulso_previne_dados_nominais.api_futuro_painel_diabeticos_lista_nominal db
