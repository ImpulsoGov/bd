...
, duplicados AS (
    SELECT 
        ...
        COUNT(DISTINCT cidadao_nome||dt_nascimento) AS cont_chaves_nominais_distintas
    -- FROM aux1 -- Adicionar CTE com o resultado final da consulta da lista nominal (transmissao, lista unificada ou painel)
    FROM impulso_previne_dados_nominais.painel_enfermeiras_lista_nominal_diabeticos 
    -- FROM impulso_previne_dados_nominais.api_futuro_painel_diabeticos_lista_nominal 
    GROUP BY 1
)
-- Checar denominador e numerador
, den_num AS (
        ...
        municipio_uf,
        -- concat(m.nome, ' - ', m.uf_sigla) AS municipio_uf,
        -- PAINEL painel_enfermeiras_lista_nominal_diabeticos
        count(DISTINCT l.cidadao_nome||l.dt_nascimento) AS diabetes_denominador,
        count(DISTINCT CASE WHEN l.possui_diabetes_autorreferida IS TRUE THEN l.cidadao_nome||l.dt_nascimento END) AS cidadaos_autorreferidos,
        count(DISTINCT CASE WHEN l.possui_diabetes_diagnosticada IS TRUE THEN l.cidadao_nome||l.dt_nascimento END) AS cidadaos_diagnosticados,
        count(DISTINCT CASE 
                            WHEN l.realizou_consulta_ultimos_6_meses IS TRUE AND realizou_solicitacao_hemoglobina_ultimos_6_meses IS TRUE 
                                THEN l.cidadao_nome||l.dt_nascimento 
        ...
        count(DISTINCT CASE WHEN l.equipe_nome_cadastro like '%SEM EQUIPE%' OR l.equipe_nome_cadastro = ' ' OR l.equipe_nome_cadastro IS NULL
                                THEN l.cidadao_nome||l.cidadao_cpf_dt_nascimento||municipio_id_sus
                        END) AS sem_equipe 
        /* PAINEL api_futuro_painel_diabeticos_lista_nominal
        ...
        count(DISTINCT CASE WHEN l.equipe_nome like '%SEM EQUIPE%' OR l.equipe_nome = ' ' OR l.equipe_nome IS NULL
                        ...
                        END) AS sem_equipe */
    FROM aux1 l
    -- FROM impulso_previne_dados_nominais.painel_enfermeiras_lista_nominal_diabeticos l
    -- FROM impulso_previne_dados_nominais.api_futuro_painel_diabeticos_lista_nominal l
    --LEFT JOIN listas_de_codigos.municipios m 
    --    ON m.id_sus = l.municipio_id_sus-- Adicionar CTE com o resultado final da consulta (transmissao, lista unificada ou painel)
    GROUP BY 1
)
